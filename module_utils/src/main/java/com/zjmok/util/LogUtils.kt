...
val isEnableLog = UtilLib.isDebug
const val TAG = "log"
const val CHUNK_SIZE = 1024

//////////////////////////////////////////////////

fun log(any: Any?, tag: String? = TAG, trace: Int = 6) {
    if (isEnableLog.not()) return
    printStackLine(trace) // 此处 trace 为 5，调用此方法处为 6
    loge(any = any, tag = tag)
}

fun logLong(any: Any?, tag: String? = TAG, trace: Int = 6) {
    if (isEnableLog.not()) return
    printStackLine(trace) // 此处 trace 为 5，调用此方法处为 6
    loge(any = any, tag = tag)
}
 ...
 * 使用时注意调试合适的 trace 参数
 *
 * - 如果直接调用 getStackLine()，则 trace 应该设置为 4
 * - 每多一层封装，trace 就需要加 1
 */
fun getStackLine(trace: Int = 4): String {
    val stackTrace = Thread.currentThread().stackTrace // 此处 trace 为 3，调用此方法处为 4
 ...
 * 使用时注意调试合适的 trace 参数
 */
fun printStackLine(trace: Int = 5) {
    if (isEnableLog.not()) return
    val stackInfo = getStackLine(trace) // 此处 trace 为 4，调用此方法处为 5
    loge(stackInfo)
}
...
fun logvLong(any: String?, tag: String? = TAG, prettyFormat: Boolean = true) {
    ...
    val list = string2List(message, 7, prettyFormat)
    list.forEach {
        Log.v(tag, it)
    }
}
...
private fun string2List(message: String, trace: Int = 6, prettyFormat: Boolean = true): List<String> {
 ...
 * 格式化长日志
 */
private fun prettyFormat(message: String, trace: Int = 5): List<String> {
